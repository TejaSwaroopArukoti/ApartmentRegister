...
const EntrySchema = yup.object({
  guestName: yup.string().matches(/^([a-zA-Z]+\s)*[a-zA-Z]+$/).required().min(4),
...
function AddEntry({navigation, route}) {
  ...
  const handleSubmitBtn = async (values, navigation) => {
    ...
    if (mode === 'EDIT') {
      let fullObj = {
        ...values,
        entryDate: otherValues.entryDate,
        id: otherValues.id,
      };
    ...
    if (route.params.mode) {
      ...
      if (route.params.mode === 'EDIT') {
        ...
        let selectedEntry = {
          ...
          vehicleNum: newEntry.vehicleNum,
          flatNum: newEntry.flatNum,
          mobileNum: newEntry.mobileNum,
          purposeOfVisit: newEntry.purposeOfVisit,
        };
